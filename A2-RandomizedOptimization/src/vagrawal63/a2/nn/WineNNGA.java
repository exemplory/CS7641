...
public class WineNNGA {
	...
	public static void main(String[] args) {
		...
		// int[] epochs = { 2, 5 };
		String fileName1 = "WineNN_GA_Train.csv";
		String fileName2 = "WineNN_GA_Test.csv";
		...
		for (int epoch : epochs) {
			...
			for (int i = 0; i < oa.length; i++) {
				for (int m = 0; m < populationSize.length; m++) {
					for (int k = 0; k < toMateSize.length; k++) {
						if (toMateSize[k] > populationSize[m]) {
							continue;
						}
						for (int l = 0; l < toMutateSize.length; l++) {
							if (toMutateSize[l] > toMateSize[k]) {
								continue;
							}
							oa[0] = new StandardGeneticAlgorithm(populationSize[m], toMateSize[k], toMutateSize[l],
									nnop[0]);
							...
							double correct = 0;
							double incorrect = 0;
							train(oa[i], networks[i], oaNames[i], epoch);
							...
							testingTime /= 1000;
							results = "";
							results = oaNames[i] + "," + epoch + "," + "[" + populationSize[m] + " - " + toMateSize[k]
									+ " - " + toMutateSize[l] + "]" + "," + df.format(correct / (correct + incorrect))
									+ "," + df.format(trainingTime) + "," + df.format(testingTime);
							FileOperationsHelper.writeToFile(fileName1, dir, results, true);
							System.out.println("======== ** NN Genetic Algorithm ** ========");
